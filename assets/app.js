...
const toNonNegativeNumber=value=>{let parsed=Number(value);return Number.isFinite(parsed)&&parsed>=0?parsed:0};
function validateToolValues(fs,values){for(let i=0;i<fs.length;i++){if(typeof fs[i][1]==='number'){if(String(values[i]).trim()==='')return `${fs[i][0]} is required.`;let parsed=Number(values[i]);if(!Number.isFinite(parsed))return `${fs[i][0]} must be a valid number.`;if(parsed<0)return `${fs[i][0]} cannot be negative.`}}return ''}
function analyzeText(text){let source=String(text||''),words=source.match(/[\p{L}\p{N}]+(?:['\u2019-][\p{L}\p{N}]+)*/gu)||[],punctuation=source.match(/[.!?]+(?=\s|$)/g)||[],paragraphs=source.trim()?source.trim().split(/\n\s*\n+/).filter(Boolean).length:0,stopWords=new Set(['the','and','for','that','with','this','from','your','you','are','was','were','have','has','had','but','not','all','can','will','into','our','their','they','its','about','than','then','when','what','who','how','why','where','which','while','also','just','more','most','some','any','each','per','use','using','used','a','an','to','of','in','on','at','by','or','as','is','it','be']),counts={};words.map(word=>word.toLocaleLowerCase()).filter(word=>word.length>2&&!stopWords.has(word)).forEach(word=>counts[word]=(counts[word]||0)+1);let keywords=Object.entries(counts).sort((a,b)=>b[1]-a[1]||a[0].localeCompare(b[0])).slice(0,5).map(([word,count])=>({word,count,density:words.length?count/words.length*100:0}));return{words:words.length,characters:[...source].length,charactersNoSpaces:[...source.replace(/\s/g,'')].length,sentences:words.length?Math.max(1,punctuation.length):0,paragraphs,readingMinutes:words.length/225,speakingMinutes:words.length/130,keywords}}
const formatDuration=minutes=>minutes===0?'0 min':minutes<1?'< 1 min':`${Math.ceil(minutes)} min`;
...
function compute(slug,a){let definitions=fields(slug),x=a.map((v,i)=>typeof definitions[i]?.[1]==='string'?String(v):toNonNegativeNumber(v)),r='';switch(slug){
...
case'automation-roi-calculator':{let save=x[0]*x[1],net=save-x[2],firstYear=net*12-x[3],investment=x[2]*12+x[3],roi=investment?firstYear/investment*100:0,payback=net>0?x[3]/net:0;r=`<strong>${money(net)} monthly net savings</strong><div class="result-breakdown"><p><b>${money(firstYear)}</b> first-year net benefit</p><p><b>${payback?payback.toFixed(1):'N/A'}</b> setup payback months</p><p><b>${roi.toFixed(1)}%</b> first-year ROI</p></div>`;break}
...
case'word-counter':{let stats=analyzeText(x[0]),keywordRows=stats.keywords.length?stats.keywords.map(item=>`<li><span>${escapeHtml(item.word)}</span><b>${item.count} · ${item.density.toFixed(1)}%</b></li>`).join(''):'<li><span>No repeated keywords yet</span><b>0%</b></li>';r=`<strong>${stats.words} words</strong><div class="word-stats"><div><span>Characters</span><b>${stats.characters}</b></div><div><span>Without spaces</span><b>${stats.charactersNoSpaces}</b></div><div><span>Sentences</span><b>${stats.sentences}</b></div><div><span>Paragraphs</span><b>${stats.paragraphs}</b></div><div><span>Reading time</span><b>${escapeHtml(formatDuration(stats.readingMinutes))}</b></div><div><span>Speaking time</span><b>${escapeHtml(formatDuration(stats.speakingMinutes))}</b></div></div><div class="keyword-density"><h3>Top keyword density</h3><ul>${keywordRows}</ul></div>`;break}
...
case'utm-builder':{try{let u=new URL(x[0]);['source','medium','campaign'].forEach((k,i)=>u.searchParams.set('utm_'+k,x[i+1]));r=`<strong>Campaign URL</strong><p style="word-break:break-all">${escapeHtml(u.toString())}</p>`}catch{r='<strong>Enter a valid destination URL, including https://</strong>'}break}
case'invoice-number-generator':r=`<strong>${escapeHtml(String(x[0]).toUpperCase())}-${new Date().getFullYear()}-${escapeHtml(String(x[1]).padStart(5,'0'))}</strong><p>Generated locally; confirm uniqueness in your records.</p>`;break;
case'business-name-generator':{let core=escapeHtml(x[0]),industry=escapeHtml(x[1]);r=`<strong>${core} ${industry} ideas</strong><p>${core} Works · ${core} Harbor · North ${core} · ${industry} Forge · ${core} &amp; Co.</p>`;break}
case'slogan-generator':{let brand=escapeHtml(x[0]),promise=escapeHtml(x[1]);r=`<strong>${brand}</strong><p>${promise}, made simpler. · Built to ${promise}. · A better way to ${promise}.</p>`;break}
...
return r}
function initTool(){const root=$('#tool-app');if(!root)return;let slug=root.dataset.slug,fs=fields(slug);root.innerHTML=`<div class="fields">${fs.map((f,i)=>{let isNumber=typeof f[1]==='number',isLongText=!isNumber&&f[0].toLowerCase().includes('text'),control=isLongText?`<textarea id="f${i}" rows="9">${escapeHtml(f[1])}</textarea>`:`<input id="f${i}" value="${escapeHtml(f[1])}" ${isNumber?'type="number" min="0" step="any" inputmode="decimal"':'type="text"'}>`;return`<label for="f${i}">${escapeHtml(f[0])}${control}</label>`}).join('')}<div class="input-error" id="input-error" role="alert" hidden></div><div class="actions"><button class="btn" id="calc" type="button">Calculate</button><button class="btn secondary" id="copy-result" type="button">Copy result</button><button class="btn secondary" id="reset-tool" type="button">Reset</button></div></div><div class="result" id="result" aria-live="polite" aria-atomic="true"><span class="muted">Enter your scenario and calculate. Your data stays in this browser.</span></div>`;let values=()=>fs.map((_,i)=>document.getElementById('f'+i).value),go=()=>{let error=validateToolValues(fs,values()),errorBox=$('#input-error');errorBox.hidden=!error;errorBox.textContent=error;if(error){$('#result').innerHTML='<strong>Check your inputs</strong><p>Fix the highlighted message, then calculate again.</p>';return}$('#result').innerHTML=compute(slug,values())};$('#calc').onclick=go;$('#copy-result').onclick=async()=>{let text=$('#result').innerText;try{await navigator.clipboard.writeText(text);$('#copy-result').textContent='Copied'}catch{$('#copy-result').textContent='Select and copy result'}setTimeout(()=>$('#copy-result').textContent='Copy result',1800)};$('#reset-tool').onclick=()=>{fs.forEach((field,i)=>document.getElementById('f'+i).value=field[1]);go()};if(slug==='word-counter'||slug==='reading-time-calculator')fs.forEach((_,i)=>document.getElementById('f'+i).addEventListener('input',go));go()}
...
function initNavigation(){document.querySelectorAll('nav').forEach(nav=>{let links=nav.querySelector('.links');if(!links||nav.querySelector('.nav-toggle'))return;let button=document.createElement('button');button.className='nav-toggle';button.type='button';button.setAttribute('aria-expanded','false');button.setAttribute('aria-label','Open navigation');button.innerHTML='<span></span><span></span><span></span>';button.addEventListener('click',()=>{let open=nav.classList.toggle('nav-open');button.setAttribute('aria-expanded',String(open));button.setAttribute('aria-label',open?'Close navigation':'Open navigation')});nav.insertBefore(button,links)})}
function initBrandIcon(){if(document.querySelector('link[rel~="icon"]'))return;let icon=document.createElement('link');icon.rel='icon';icon.type='image/svg+xml';icon.href='/assets/helmet.svg';document.head.appendChild(icon)}
function initHomeFilters(){let buttons=[...document.querySelectorAll('[data-home-filter]')],cards=[...document.querySelectorAll('[data-home-category]')];buttons.forEach(button=>button.addEventListener('click',()=>{let filterName=button.dataset.homeFilter;buttons.forEach(item=>{let active=item===button;item.classList.toggle('active',active);item.setAttribute('aria-pressed',String(active))});cards.forEach(card=>{let match=filterName==='all'||card.dataset.homeCategory.split(' ').includes(filterName);card.hidden=!match})}))}
document.addEventListener('DOMContentLoaded',()=>{initBrandIcon();initTool();initNavigation();initHomeFilters();$('#catalog-search')?.addEventListener('input',filter)})
